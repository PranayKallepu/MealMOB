...
const vendorRegister = async (req, res) => {
    try {
        ...
        // create new vendor
        const newVendor = await Vendor.create({vendorName, vendorEmail, vendorPassword: hashedPassword})
        await newVendor.save()
        // return response
        return res.status(200).json({success: true, vendorName, message: 'Vendor registered successfully'})
    } catch (error) {
...
const vendorLogin = async (req, res) => {
    try {
        ...
        const isPasswordMatch = await bcrypt.compare(vendorPassword, vendor.vendorPassword)
        // check password
        if(!isPasswordMatch){
        ...
        // return response
        return res.status(200).json({success: true, vendorId: vendor._id, restaurantId: restaurant._id || null,  message: 'Vendor logged in successfully', token})
    } catch (error) {
